...
可以对南宁的文白异读初步进行一个分类，因为不同的文白异读使用场景有条件，为了简化叙述，斜杠左边文读右边白读，文读韵腹指前高元音，声母指清浊擦音、无擦通音及半元音、舌根不送气清塞音或双唇塞音，声调指阴调，白读韵腹指后低元音，声母指舌根送气清塞音或双唇鼻音，声调指阳调：

1).只读白读（文读只是可读）

文白 | 字
---|---
文[-a-]白[-ɛ-] | 猫斩减插闲八滑挖教抄喊拗詏刨
...
文白 | 字
---|---
文[-a-]白[-ɛ-] | 关（姓）/（动词），翻（翻新）/（翻开），包（包办，包公，面包）/（打包，麻包，皮包），夹（夹层）/（铁夹；夹住），咸（咸阳）/（咸淡），坳（高峰坳，地名）/（山坳）
